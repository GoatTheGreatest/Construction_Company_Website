...
function ChartTooltipContent({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }) {
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const value = !labelKey && typeof label === "string" ? (config && config[label])?.label || label : itemConfig?.label;
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  {itemConfig?.icon ? (
                    ...
                    !hideIndicator && (
                      <div className={cn("shrink-0 rounded-[2px] border-(--color-border) bg-(--color-bg)", indicator === "dot" ? "h-2.5 w-2.5" : indicator === "line" ? "w-1" : "w-0 border-[1.5px] border-dashed bg-transparent")}
                        style={{
